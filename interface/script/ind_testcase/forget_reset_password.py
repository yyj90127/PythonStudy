...
@ddt.ddt
class interface_forget_reset_password(ind_Base):
    def setUp(self):
        self.URL = f'{self.BaseURL}forget_reset_password.do'

    table = readcsv('ind', 'forget_reset_password_data.csv')
    @ddt.data(*table)
...
if __name__ == '__main__':
    suite = unittest2.defaultTestLoader.discover(CUR_DIR,'forget_reset_password.py')
    filePath = PKG_DIR.replace('script','result/ind_result/forget_reset_password_result.html')
