...
class interface_forget_get_question(object):
    def __init__(self):
        list = []
        table = readcsv('url')
        for i in table:
            list.append(i)
        self.interface_name = 'forget_get_question'
        ...
        with open(filePath, 'w', encoding='utf-8') as f:
            for i in self.getdata():
                ...
                if str(result).find('0') > 0:
                    f.write(f'用户"{i[0]}"获取问题成功：{result["data"]}\n')
                else:
                    f.write(f'用户"{i[0]}"获取问题失败：{result["msg"]}\n')


if __name__ == '__main__':
    interface_forget_get_question().result()