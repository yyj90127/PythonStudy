...
@ddt.ddt
class interface_login(ind_Base):
    def setUp(self):
        self.URL = f'{self.BaseURL}login.do'

    table = readcsv('ind', 'login_data.csv')
    @ddt.data(*table)
...
if __name__ == '__main__':
    suite = unittest2.defaultTestLoader.discover(CUR_DIR,'login.py')
    filePath = PKG_DIR.replace('script','result/ind_result/login_result.html')
